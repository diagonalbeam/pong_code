...
<template>
  ...
  >
    ...
    >
      ...
      <button
        class="m-2 flex min-h-10 w-[calc(100%-16px)] cursor-pointer items-center gap-2.5 rounded-[6px] border-0 bg-transparent px-3 text-left text-sm text-[var(--pc-text-secondary)] hover:bg-[var(--pc-surface-soft)] hover:text-[var(--pc-text)]"
        type="button"
        :aria-label="collapsed ? '展开侧栏' : '收起侧栏'"
        @click="collapsed = !collapsed"
      >
        <el-icon class="w-5 shrink-0 text-lg"><Expand v-if="collapsed" /><Fold v-else /></el-icon>
      ...
      <header class="sticky top-0 z-30 flex h-[var(--pc-header-height)] items-center justify-between gap-2 overflow-hidden border-b border-[var(--pc-border)] bg-[var(--pc-header)] px-[17px] max-md:pr-3 max-md:pl-2">
        ...
        <div class="flex shrink-0 items-center gap-2 pr-1">
          ...
          <el-badge :value="3" :max="99">
            <el-button circle text aria-label="通知" @click="ElMessage.info('通知功能开发中')">
              <el-icon><Bell /></el-icon>
            </el-button>
          </el-badge>
          <el-dropdown trigger="click">
            <el-button text class="min-h-10 pr-2.5 pl-1.5">
              <el-avatar :size="32">
                {{ auth.user?.username?.slice(0, 1).toUpperCase() }}
              </el-avatar>
              <span class="max-w-[120px] overflow-hidden text-ellipsis max-md:hidden">{{ auth.user?.username }}</span>
            </el-button>
            <template #dropdown>
              <el-dropdown-menu>

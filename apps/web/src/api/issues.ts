...
export function createIssue(projectId: number, data: Record<string, unknown>) {
  return http.post<Issue>(`/projects/${projectId}/issues`, data)
}

export function getIssue(id: number) {
  return http.get<{ issue: Issue; work_logs: WorkLog[] }>(`/issues/${id}`)
}
...
export function addIssueWorklog(id: number, data: Record<string, unknown>) {
  return http.post<{ log: WorkLog; issue: Issue }>(`/issues/${id}/worklogs`, data)
}

export function deleteIssueWorklog(id: number, worklogId: number) {
  return http.delete<{ success: boolean; issue_id: number }>(`/issues/${id}/worklogs/${worklogId}`)
}

export function moveIssue(id: number, status: string) {
  return http.post<{ success: boolean }>(`/issues/${id}/move`, { status })
}

export function assignIssueSprint(id: number, sprintId: number | null) {
  return http.post<{ success: boolean }>(`/issues/${id}/assign_sprint`, { sprint_id: sprintId })
}

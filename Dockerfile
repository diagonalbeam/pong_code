...
# - GUNICORN_KEEP_ALIVE（默认 620 秒，高于 Google 负载均衡的 600 秒后端空闲超时）
...
CMD ["sh", "-c", "gunicorn --chdir apps/api -w ${WEB_CONCURRENCY:-2} -k gthread --threads ${GUNICORN_THREADS:-4} --keep-alive ${GUNICORN_KEEP_ALIVE:-620} -b 0.0.0.0:${PORT:-5000} --log-level ${LOG_LEVEL:-info} --access-logfile - --error-logfile - app:app"]
